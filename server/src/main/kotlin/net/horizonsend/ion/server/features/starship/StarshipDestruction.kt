...
object StarshipDestruction {
	...
	fun vanish(data: PlayerStarshipData) {
		val air = Material.AIR.createBlockData().nms

		var state = DeactivatedPlayerStarships.getSavedState(data)

		if (state == null) {
			try {
			    state = StarshipDetection.detectNewState(data)
			} catch (error: StarshipDetection.DetectionFailedException) {
				IonServer.logger.warning(error.message)
			}
		}

		state ?: return

		val queue = Long2ObjectOpenHashMap<BlockState>(state.blockMap.size)
		state.blockMap.keys.associateWithTo(queue) { air }
		BlockPlacement.placeImmediate(data.bukkitWorld(), queue)
	}
